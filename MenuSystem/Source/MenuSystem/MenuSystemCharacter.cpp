...
void AMenuSystemCharacter::CreateGameSession()
{
	...
	const TSharedPtr<FOnlineSessionSettings> SessionSetting = MakeShareable(new FOnlineSessionSettings());
...
void AMenuSystemCharacter::OnCreateSessionComplete(FName SessionName, bool bIsSuccessful)
{
	if(bIsSuccessful)
	{
		if(GEngine)
		{
			GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Blue, FString::Printf(TEXT("Craete session %s"), *SessionName.ToString()));
		}	
	}
	else
	{
